...
class CSpiPrg:
    ...
    def MmioRead (self, addr, dlen):
        return self.DownloadData (addr, dlen)

    def MmioWrite (self, bins):
        # Write data to DPRAM
        lpkt = 0x200
        spic = SPI_PKT(1, CSpiPrg.CMD_OFF)
        cmdb = CMD_PKT(9, 0, lpkt)
    ...
    def SysDbgSel (self, dbgsel):
        # Write control
        spid = SPI_PKT(7, dbgsel)
        data = bytearray(spid)
        self.spi.exchange (data, 0)
    ...
    def SysStat (self):
        ...
        data   = bytearray(spid)
        rdata  = bytearray()
        rdata += self.spi.exchange (data, 16)
        return rdata
    ...
    def CpuPc (self):
        ...
        data   = bytearray(spid)
        rdata  = bytearray()
        rdata += self.spi.exchange (data, 4)
        return rdata
...
def Main():
    ...
    if 'v' in flags:
        # Dump CPU state
        regs = ['pc', 'mepc', 'mstatus', 'mcause']
        for i, reg in enumerate(regs):
          SpiPrg.SysDbgSel (i)
          data = SpiPrg.SysStat ()
          print ("%-8s: %08x" % (reg, Bytes2Val(data[8:12])))
        #print_bytes (data)
