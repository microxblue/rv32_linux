...
static uint32_t HandleControlStore( uint32_t addy, uint32_t val )
{
	if (addy == 0xffff0400) //UART 8250 / 16550 Data Buffer
	{
		printf( "%c", val );
		fflush( stdout );
	}
...
static uint32_t HandleControlLoad( uint32_t addy )
{
	...
	if (addy == 0xffff0414)
		return 0x60 | IsKBHit();
	else if (addy == 0xffff0400 && IsKBHit())
		return ReadKBByte();
...
int main(int argc, char** argv)
{
	...
	if (argc > 1) {
		...
		fprintf(stderr, "Uasage:\n  rv32_emu kernel_image\n");
		return 1;
	}
...
restart:
	{
		...
		fseek(f, 0, SEEK_END);
		uint32_t flen = (uint32_t)ftell(f);
		fseek(f, 0, SEEK_SET);
		if (flen > ram_amt)
		{
		...
		// Load a default dtb.
		dtb_ptr = ram_amt - 0x40000;
		memcpy(ram_image + dtb_ptr, (void*)(uintptr_t)dtb_bin, sizeof(dtb_bin));
	}
	...
	for (rt = 0; rt < (uint64_t)instct + 1 || instct < 0; rt += instrs_per_flip)
	{
		uint64_t* this_ccount = ((uint64_t*)&core->cyclel);
		uint32_t elapsedUs = 0;
		if (fixed_update)
			elapsedUs = (uint32_t)(*this_ccount / time_divisor - lastTime);
		else
			elapsedUs = (uint32_t)(GetTimeMicroseconds() / time_divisor - lastTime);
		lastTime += elapsedUs;
