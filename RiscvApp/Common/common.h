...
#define  MMIO_B_AND_OR(x, a, b)  MMIO_B(x) = (MMIO_B(x) & (a)) | (b)
#define  MMIO_S_AND_OR(x, a, b)  MMIO_S(x) = (MMIO_S(x) & (a)) | (b)
...
int  test (uint32_t arg0, uint32_t arg1);
...
void  setup_timer (uint32_t ticks);
...
#define CSRR_READ(csr, v)  __asm__ __volatile__ ("csrr %0, " STRINGIZE(csr) : "=r" (v))
#define CSRR_WRITE(csr, v) __asm__ __volatile__ ("csrw " STRINGIZE(csr) ", %0" : : "r" (v))
